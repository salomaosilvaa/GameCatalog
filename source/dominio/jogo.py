...
class Jogo:
    ...
    def adicionar_horas(self, qtd: float):
        if qtd < 0:
            raise ValueError("Não é possível adiconar horas negativas.")
        self._horas += qtd
    ...
    def finalizar(self):
        if self._horas < 1:
            raise ValueError("Não é possível finalizar um jogo com menos de 1 hora de gameplay.")
    ...
    def __repr__(self):
        return (f"Jogo(titulo={self.titulo!r}, genero={self.genero!r}, "
